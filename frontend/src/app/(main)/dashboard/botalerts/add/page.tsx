...
export default function AddUserPage() {
  ...
  const [models, setModels] = React.useState({
    evergreen: { enabled: true, expiry: "2026-10-24" },
    legacy: { enabled: false, expiry: "" },
    alpha: { enabled: false, expiry: "" },
  });
  const [selectedSymbols, setSelectedSymbols] = React.useState<Set<string>>(new Set(BASIC_PACKAGE));
  const [selectedTimeframes, setSelectedTimeframes] = React.useState<Set<string>>(new Set(["15M", "1H"]));
  const [packageName, setPackageName] = React.useState("Quick Packages");
  const [timeframeName, setTimeframeName] = React.useState("Timeframe Presets");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // 1. Grab HTML checkboxes
    const formElement = e.currentTarget;
    const formData = new FormData(formElement);

    // 2. Extract specific checkbox arrays
    ...
    // 3. Build the payload mapped to model prefixes
    ...
    try {
      ...
      if (response.ok && responseData.success) {
        ...
        alert(`Failed to add user: ${responseData.message || response.statusText}`);
      }
    } catch (error) {
      ...
      <div className="max-w-5xl mx-auto p-2 sm:p-6 lg:p-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-2">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-foreground">Add New User</h1>
          </div>
          <p className="text-muted-foreground text-sm sm:ml-[56px]">
            Create a new user profile, generate keys, and configure algorithmic access tiers.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <CardContent className="p-0">
              ...
              <div className="overflow-hidden rounded-lg border border-border">
                <Table>
                  ...
                  <TableBody>
                    {ENTRY_TYPES.map((row) => {
                      const isDefaultChecked = ["CR", "BRK", "CISD"].includes(row.id);
                      return (
                        <TableRow key={row.id} className="hover:bg-muted/50 transition-colors border-border">
                          ...
                          <TableCell className="py-3 text-center">
                            <div className="flex justify-center">
                              {/* IMPORTANT: Added 'name' attribute here for FormData extraction */}
                              <Checkbox
                                id={`entry_${row.id}`}
                                name={`entry_${row.id}`}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 bg-muted/30 p-4 rounded-xl max-h-[400px] overflow-y-auto border border-border">
                  {MODIFIERS.map((mod, i) => (
                    <div
                      ...
                      className="flex items-center p-2 bg-background rounded-lg hover:bg-accent hover:text-accent-foreground hover:translate-x-1 transition-transform border border-border shadow-sm"
                    >
                      {/* IMPORTANT: Replaced raw string map with exact name attributes for Flask */}
                      <Checkbox id={mod.id} name={mod.id} defaultChecked={i < 2} className="mr-2" />
              ...
              <CardContent className="p-0">
                <div className="flex flex-wrap gap-3 bg-muted/30 p-4 rounded-xl border border-border">
                  <div className="w-fit flex items-center p-2 bg-background rounded-lg hover:bg-accent hover:text-accent-foreground hover:translate-x-1 transition-transform border border-border ">
                    {/* IMPORTANT: Added precise 'name' attributes for Flask processing */}
                    <Checkbox id="trend_bull" name="trend_Bull" defaultChecked className="mr-2" />
                  ...
                  <div className="w-fit flex items-center p-2 bg-background rounded-lg hover:bg-accent hover:text-accent-foreground hover:translate-x-1 transition-transform border border-border ">
                    {/* IMPORTANT: Added precise 'name' attributes for Flask processing */}
                    <Checkbox id="trend_bear" name="trend_Bear" defaultChecked className="mr-2" />
          ...
          {/* MAX-VISIBILITY Floating Add User Button */}
