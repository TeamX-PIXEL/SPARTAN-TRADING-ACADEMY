...
export const getBotColumns = ({ onEdit, onDelete }: BotColumnsProps): ColumnDef<BotRow, any>[] => [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const src = thumb ? (thumb.startsWith("http") ? thumb : `${API_URL}${thumb}`) : null;
      return src ? (
        <img src={src} alt={row.original.bot_name} className="h-10 w-10 rounded-md object-cover border" />
      ) : (
