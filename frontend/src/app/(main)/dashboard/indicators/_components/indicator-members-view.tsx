...
export function IndicatorMembersView({ indicator }: Props) {
  ...
  const pageCount = Math.ceil(indicatorMembers.length / pageSize);
  const pagedMembers = indicatorMembers.slice(pageIndex * pageSize, (pageIndex + 1) * pageSize);
  ...
  const paid = indicatorMembers.filter((m) => m.accessType === "paid").length;
  const free = indicatorMembers.filter((m) => m.accessType === "free").length;
  ...
  function handleOpenExpiryEdit(member: Member) {
    ...
    setExpiryValue(member.indicatorExpiry || "");
  ...
  if (!indicator) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
        ...
        <Button variant="outline" onClick={goIndicators}>
          <ArrowLeft className="mr-2 size-4" /> Back to Indicators
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full min-w-0 flex-col gap-4 md:gap-6">
      <div className="flex flex-col gap-3">
        <Button variant="ghost" size="sm" className="w-fit -ml-2 text-muted-foreground" onClick={goIndicators}>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              <span className="font-medium text-foreground">{indicator.title}</span>{" "}
              <span className="font-mono text-[11px]">({indicator.indicator_id})</span> — manage purchaser
        ...
        <CardHeader className="flex flex-row items-center justify-between gap-2 pb-0">
          <div className="flex flex-col space-y-1 overflow-hidden">
            ...
            <CardDescription className="hidden truncate sm:block">
              Manage purchaser access and contact details for this indicator.
            </CardDescription>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex-1 text-muted-foreground text-sm">
                {indicatorMembers.length} row(s) total.
              </div>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Grant a client access to <span className="font-medium text-foreground">{indicator.title}</span>.
              Search by @username to find a registered user.
            </DialogDescription>
          </DialogHeader>

          <div className="grid gap-4 py-2">
            <div className="space-y-2">
              <Label htmlFor="im-search">Search User</Label>
      ...
      <Dialog open={expiryEditOpen} onOpenChange={setExpiryEditOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Calendar className="size-5" /> Edit Indicator Expiry
            </DialogTitle>
            <DialogDescription>
              Set the indicator access expiry for <span className="font-medium text-foreground">{expiryEditMember?.name}</span>.
            </DialogDescription>
          </DialogHeader>
          <div className="py-2">
            <div className="space-y-2">
              <Label>Indicator Expiry Date</Label>
              <Input
                type="date"
                value={expiryValue}
