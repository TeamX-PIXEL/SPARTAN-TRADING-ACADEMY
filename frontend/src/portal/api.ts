...
export const API = {
  ...
  async getBots(skip: number = 0, limit: number = 2): Promise<{ items: Bot[]; hasMore: boolean }> {
    ...
    const items: Bot[] = (Array.isArray(data) ? data : data.bots || []).map((b: any) => ({
      ...
      longDescription: b.long_description || b.description,
      price: b.price,
      image: b.image || "",
  ...
  async getLibrary(): Promise<{ courses: Course[]; indicators: Indicator[]; bots: Bot[] }> {
    ...
    return {
      courses: (d.courses || []).map((c: any) => ({
        ...
        category: "Masterclass",
        features: [],
        duration: c.duration_months ? `${c.duration_months} Month${c.duration_months > 1 ? 's' : ''}` : "1 Month",
        lecturer: c.lecturer || "TBA",
        difficulty: c.difficulty || "Beginner" as const,
        scheduled_at: c.scheduled_at,
        discord_renewal_price: c.discord_renewal_price ?? undefined,
      ...
      indicators: (d.indicators || []).map((i: any) => ({
        ...
        description: i.description || "",
        longDescription: i.long_description || i.description || "",
        price: 0,
